...
public class PeerFetcher extends Thread {
    ...
    //logger
    public static Logger logger = org.apache.log4j.Logger.getLogger(PeerFetcher.class);
        ...
        while(is_running.get()) {
            ...
            if(EdgeKeeper.started) {
                ...
                if (peerGUIDs != null) {
                    ...
                    Set<String> guids = Utils.SharedPreferences_keys(MDFS_PEERS_SHARED_PREF);

                    if (guids != null) {

                        logger.log(Level.ALL, "List of <GUID-Names> already in SP: " + guids);
                        ...
                        for (int i = 0; i < peerGUIDs.size(); i++) {
                            ...
                            if (!guids.contains(guid)) {
                                ...
                                if (name != null) {
                                    ...
                                    Utils.SharedPreferences_put(MDFS_PEERS_SHARED_PREF, guid, name);
                                }
                            }
                        }
                    } else {
    ...
    //list can be empty
    public static List<String> getCurrentPeersGUIDs(){
    ...
    //list can be empty.
    public static List<String> getCurrentPeersNames(){
    ...
    public static String GUIDtoNameConversion(String GUID){

        //fetch value(name) for this key(guid)
        String name = Utils.SharedPreferences_get(MDFS_PEERS_SHARED_PREF, GUID);
